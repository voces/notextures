...
const minNotNegInfinity = ( ...arr ) => {
	...
	const value = arr.reduce( ( min, val ) =>
		val === - Infinity ? min : val < min ? val : min, Infinity );
		...
		for ( let y = this.height - 1; y >= 0; y -- )
			for ( let x = 0; x < this.width; x ++ ) {
				...
				if ( ! isNaN( cliffMask[ y ][ x ] ) ) {
					...
					geometry.faces.push(
						new Face3(
							...tileFaceVertices( vertices, true ),
							undefined,
							this.groundColor( x, y ),
						),
						new Face3(
							...tileFaceVertices( vertices, false ),
							undefined,
							this.groundColor( x, y ),
						),
					);
					...
					if ( x > 0 ) {
						...
						for ( let z = low; z < high; z ++ ) {
							...
							geometry.faces.push(
								new Face3(
									...wallVertices( vertices, true, currentIsLow, true ),
									undefined,
									this.cliffColor( x, y ),
								),
								new Face3(
									...wallVertices( vertices, true, currentIsLow, false ),
									undefined,
									this.cliffColor( x, y ),
								),
							);
					...
					if ( y > 0 ) {
						...
						for ( let z = low; z < high; z ++ ) {
							...
							geometry.faces.push(
								new Face3(
									...wallVertices( vertices, false, currentIsLow, true ),
									undefined,
									this.cliffColor( x, y ),
								),
								new Face3(
									...wallVertices( vertices, false, currentIsLow, false ),
									undefined,
									this.cliffColor( x, y ),
								),
							);
					...
					for ( let i = 0; i < walls.length; i ++ ) {
						...
						if ( a.z !== b.z && ( a.x === b.x || a.y === b.y ) ) {
							...
							rampWalls.push(
								new Face3( a._geoIndex, b._geoIndex, v._geoIndex, undefined, this.cliffColor( x, y ) ),
							);

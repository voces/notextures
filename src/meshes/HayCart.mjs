...
export default class HayCart extends Mesh {
	...
	constructor() {
		...
		const geometry = new Builder()
			// Wheels
			.repeat( 2, ( b, x ) => b.cylinder( 1 / 4, 1 / 4, 1 / 16 )
				.translate( x, 0, 7 / 32 )
				.rotateZ( Math.PI / 2 )
				.color( wheels ) )
			// Axis
			...
			.parent
			// Base
			.box( 1 / 2, 1 / 2, 1 / 32 ).translateZ( 1 / 4 )
			.color( basket )
			.parent
			// Walls
			.thickLathe( [
			...
			], 1 / 32, new Vector2( - 1, 0 ), 4, Math.PI / 4 ).rotateX( Math.PI / 2 )
			.color( basket )
			.parent
			// Handles
			...
			.for( 3, ( b, z ) =>
				b.sphere( 1 / 3 - z / 24, 8, 6, 0, Math.PI * 2, 0, Math.PI / 2 )
					.rotateX( Math.PI / 2 )
					.translateZ( 1 / 4 + Array( z ).fill()
						.reduce( ( sum, _0, i ) => sum + ( 1 / 3 - i / 24 ) / 2, 0 ) )
					.color( hay ),
			)
			.root()
			.randomize()
